...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // ---- Strip hash on REFRESH only, so navigating from reviews still lands on #libri ----
  ...
  chips.forEach(chip => {
    chip.addEventListener('click', () => {
      ...
      let visibleCount = 0;
      bookCards.forEach(card => {
        const cardGenres = card.dataset.genre ? card.dataset.genre.toLowerCase().split(/\s+/) : [];
        const searchQuery = searchInput ? searchInput.value.toLowerCase().trim() : '';
        const matchesGenre = activeGenre === 'tutti' || cardGenres.includes(activeGenre.toLowerCase());
